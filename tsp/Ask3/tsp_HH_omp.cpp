...
{
	bool flag = false;
	//#pragma omp parallel for reduction(or:flag)
...
int find_min_of_row(int row, int explored_ind, int explored[])
{
	...
	int ind = -1;
	#pragma omp parallel for shared(min_d, ind) //private(explored_ind, explored, row)//, dists)
	for(int j = 0; j < N; j++)
	{
...
int find_second_min_of_row(int row, int explored_ind, int explored[], int j1)
{
	double min_d = 100000.0;
	int ind = -1;
	#pragma omp parallel for shared(min_d, ind) //, dists)
